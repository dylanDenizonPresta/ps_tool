...
# Désinstallation du module ps_accounts
...
# Charger la fonction de reset
if [ -f "${SCRIPT_DIR}/commands/account/reset.sh" ]; then
    source "${SCRIPT_DIR}/commands/account/reset.sh"
fi

# Commande principale pour désinstaller ps_accounts
cmd_account_uninstall() {
    # Afficher l'aide si demandé
    if [ $# -eq 0 ] || [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
        cat << EOF
Désinstallation du module ps_accounts d'un shop PrestaShop

Usage:
    ps_tool account uninstall <nom_shop> [options]

Arguments:
    nom_shop           Nom de la shop où désinstaller ps_accounts

Options:
    --files            Supprimer aussi les fichiers du module (modules/ps_accounts/)
...
La commande va:
    1. Réinitialiser les données de configuration ps_accounts (suppression des clés de config)
    2. Désinstaller le module ps_accounts via la console Symfony
    3. Optionnellement supprimer les fichiers du module si --files est spécifié
EOF
        if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
            return 0
        else
    ...
    info "Désinstallation de ps_accounts du shop: $shop_name"
    info "Chemin du shop: $shop_path"
    
    # Vérifier que le module ps_accounts est installé
    local accounts_module_path="${shop_path}/modules/ps_accounts"
    if [ ! -d "$accounts_module_path" ]; then
        warning "Le module ps_accounts n'est pas installé dans ce shop"
        return 0
    fi
    ...
    if ! ddev describe > /dev/null 2>&1; then
        ...
        if confirm "Voulez-vous démarrer ddev maintenant ?"; then
            ...
            if [ "$remove_files" = true ]; then
                ...
                if rm -rf "$accounts_module_path"; then
    ...
    # Réinitialiser les données de configuration avant la désinstallation
    info "Réinitialisation des données de configuration ps_accounts..."
    if function_exists "cmd_account_reset"; then
        # Appeler la fonction de reset mais sans demander de confirmation supplémentaire
        # On va exécuter directement la requête SQL
        ...
        if [ $reset_exit_code -eq 0 ]; then
            success "Données de configuration ps_accounts supprimées"
        else
            warning "Échec de la suppression des données de configuration (continuation de la désinstallation)"
            echo "$reset_output" | tail -5 >&2
        fi
    else
        warning "Fonction de reset non disponible, continuation de la désinstallation"
    fi
    
    # Désinstaller le module via la console Symfony
    info "Désinstallation du module via la console Symfony..."
    local uninstall_output
    uninstall_output=$(ddev exec "php bin/console prestashop:module uninstall ps_accounts 2>&1" 2>&1)
    local uninstall_exit_code=$?
    
    if [ $uninstall_exit_code -eq 0 ]; then
        success "Module ps_accounts désinstallé avec succès"
    else
        ...
        if echo "$uninstall_output" | grep -qiE "not installed|non installé|not found"; then
            info "Le module ps_accounts n'est pas installé"
            ...
            # Vraie erreur
            error "Échec de la désinstallation du module ps_accounts"
    ...
    success "Désinstallation de ps_accounts terminée avec succès !"
    return 0
}
